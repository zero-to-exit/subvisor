...
class Algorithm1:
    ...
    def extract_all_frames(self, ratio = 0.1) -> None:
        '''
        ...
        output:
        None (frames are saved to self.total_frame_dir)
        '''
        if not (0 < ratio <= 1.0):
    ...
    def extract_frame_number(fp: Path) -> int:
        ...
        if name.startswith("f") and name[1:].isdigit():
            return int(name[1:])  # f123 -> 123
        elif name.isdigit():
            return int(name)  # 123 -> 123
        else:
            # Try to extract from legacy frame_0123 format
            return int(name.split("_")[-1]) if "_" in name else int(name)
    ...
    def divide_frames_into_groups(self, scene_threshold: float = 0.5, group_count: int = 30):
        ...
        group_count: target minimum number of groups

        output:
        groups: dictionary of group id and list of frame numbers
        '''
        ...
        frame_paths = sorted(frame_paths, key=lambda p: self.extract_frame_number(p))
        ...
        for fp in frame_paths:
            ...
            frame_num = self.extract_frame_number(fp)
        ...
        min_group_size = 3  # Minimum frames per group before merging
        merged = []
        for g in groups_list:
            if merged and len(g) < min_group_size:
                merged[-1].extend(g)
            else:
                merged.append(g)

        # Adjust group count to match target group_count
        target_groups = group_count
        
        # If we have fewer groups than target, split large groups
        if len(merged) < target_groups:
            while len(merged) < target_groups:
        ...
        # If we have more groups than target, merge smallest groups
        elif len(merged) > target_groups:
            while len(merged) > target_groups:
                # Find the smallest group (prefer merging with adjacent groups)
                smallest_idx = min(range(len(merged)), key=lambda i: len(merged[i]))
                
                # Decide which adjacent group to merge with
                if smallest_idx == 0:
                    # Merge with next group
                    merge_idx = 1
                elif smallest_idx == len(merged) - 1:
                    # Merge with previous group
                    merge_idx = smallest_idx - 1
                else:
                    # Merge with smaller adjacent group
                    prev_size = len(merged[smallest_idx - 1])
                    next_size = len(merged[smallest_idx + 1])
                    merge_idx = smallest_idx - 1 if prev_size <= next_size else smallest_idx + 1
                
                # Merge groups (maintain time order)
                if merge_idx < smallest_idx:
                    merged[merge_idx].extend(merged[smallest_idx])
                    merged.pop(smallest_idx)
                else:
                    merged[smallest_idx].extend(merged[merge_idx])
    ...
    def score_frames(self, target_count: int = 30, per_group_selected: int = 1, yolo_weights: str = "yolov8n.pt"):
        ...
        for gid, paths in groups.items():
            ...
            for fp in paths:
                ...
                # Extract frame number from path using common function
                frame_num = self.extract_frame_number(fp)
